...
`ws <name>` offers Create and return here, then Create and stay there, below any partial desktop/action matches. An exact case-insensitive desktop-name match hides these Create rows. New workspace… always offers creation, including duplicate desktop names. Both screens show the terminal link and configured apps: attach an existing tmux session, create in a matching project folder, or create a fresh home session. Browser new tab and blank notes defaults stay unchanged. See [terminal matching and configuration](../new-workspace/README.md#configuration). Advanced directory, URL, note and existing-session options remain in `workspace create --help`. Failed terminal/folder lookup disables Create while desktop actions remain usable; the explicit-directory opt-out is available in the CLI.
